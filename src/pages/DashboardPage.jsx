...
import { fetchStats, fetchRevenue, fetchPerformance, getClients, fetchPtSummary, getSupplementDashboardSummary, getPtAssignments } from '../api';
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [statsData, revenueData, perfData, clientsData, ptData, suppSummaryData, ptAssignData] = await Promise.all([
          ...
          <div className="cards-grid-4">
            ...
            <div className="ui-dash-card" onClick={() => navigate('/transactions')}>
              <div className="card-top-row">
                <span className="card-category-title">MONTHLY COLLECTION</span>
              </div>
              <div className="card-val-row">
                <div className="card-main-val">{formatCurrency(stats?.monthlyCollection)}</div>
              </div>
            ...
            <div className="ui-dash-card" onClick={() => navigate('/expenses')}>
              <div className="card-top-row">
                <span className="card-category-title">MONTHLY EXPENSES</span>
              </div>
              <div className="card-val-row">
                <div className="card-main-val card-val-red">{formatCurrency(stats?.monthlyExpenses)}</div>
              </div>
              <div className="card-bottom-row">
                <span className="card-link-action">View Details</span>
            ...
            <div className="ui-dash-card" onClick={() => navigate('/transactions')}>
              ...
              <div className="card-val-row">
                <div className="card-main-val card-val-green">{formatCurrency(stats?.netProfit)}</div>
              </div>
              <div className="card-bottom-row">
                <span className="card-link-action">View Details</span>
                <div className="card-icon-badge badge-green">
            ...
            <div className="ui-dash-card combined-status-card" style={{ gridColumn: 'span 2' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                {/* Row 1: Client Memberships (4 metrics) */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.6rem' }}>
                  <div className="combined-metric-box" style={{ background: '#f0fdf4', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #bbf7d0', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/manage-clients?status=Active')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#16a34a', lineHeight: 1 }}>{stats?.activeClients || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#15803d', textTransform: 'uppercase', marginTop: '0.35rem' }}>Active Clients</div>
                  </div>

                  <div className="combined-metric-box" style={{ background: '#fdf2f8', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #fbcfe8', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/pt-assignments?status=Active')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#db2777', lineHeight: 1 }}>{activePtCount || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#be185d', textTransform: 'uppercase', marginTop: '0.35rem' }}>Active PT</div>
                  </div>

                  <div className="combined-metric-box" style={{ background: '#fffbeb', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #fde68a', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/manage-clients?status=Inactive')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#d97706', lineHeight: 1 }}>{stats?.inactiveClients || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#b45309', textTransform: 'uppercase', marginTop: '0.35rem' }}>Inactive Clients</div>
                  </div>

                  <div className="combined-metric-box" style={{ background: '#fef2f2', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #fecaca', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/manage-clients?status=Expired')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#dc2626', lineHeight: 1 }}>{stats?.expiredClients || stats?.expiredPlans || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#b91c1c', textTransform: 'uppercase', marginTop: '0.35rem' }}>Expired Clients</div>
                  </div>
                </div>

                {/* Row 2: Advance Bookings & Inactive PT (3 metrics) */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.6rem' }}>
                  <div className="combined-metric-box" style={{ background: '#eeF2ff', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #c7d2fe', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/advance-bookings?tab=general')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#4f46e5', lineHeight: 1 }}>{stats?.generalAdvanceBookings || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#4338ca', textTransform: 'uppercase', marginTop: '0.35rem' }}>General Advance</div>
                  </div>

                  <div className="combined-metric-box" style={{ background: '#fae8ff', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #f5d0fe', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/advance-bookings?tab=pt')}>
                    <div style={{ fontSize: '1.4rem', fontWeight: '900', color: '#c026d3', lineHeight: 1 }}>{stats?.ptAdvanceBookings || 0}</div>
                    <div style={{ fontSize: '0.72rem', fontWeight: '800', color: '#a21caf', textTransform: 'uppercase', marginTop: '0.35rem' }}>PT Advance</div>
                  </div>

                  <div className="combined-metric-box" style={{ background: '#fff1f2', padding: '0.75rem 0.4rem', borderRadius: '12px', border: '1px solid #fecdd3', cursor: 'pointer', textAlign: 'center' }} onClick={() => navigate('/pt-assignments?status=Inactive')}>
          ...
          <div className="recent-feed-column">
            <section className="feed-section recent-feed-card">
              ...
              <div className="dynamic-list-scroll">
                {stats?.transactions?.slice(0, 6).map((txn, i) => (
